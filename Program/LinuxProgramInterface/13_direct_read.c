...
int main(int argc, char* argv[])
{
	...
	/* memaligh() allocates a block of memory aligned on address that is a multiple of its first argument */
	/* The following expression ensures that 'buf' is aligned on a non-power-of-two multiple of 'alignment'*/
	/* We do this to ensure that if, for example, we ask for a 256-byte aligned buffer, then we don't */
	/* accidentally get a buffer that is also aligned on a 512-byte boundary */
