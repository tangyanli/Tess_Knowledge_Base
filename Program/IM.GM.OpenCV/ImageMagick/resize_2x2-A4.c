...
int main(int argc, char **argv)
{
	...
	*/
	...
	image_info->depth = 8;
	fun_CloneString(&(image_info->size), "4725x6780");
    ...
    */
	...
	crop_info.width = 2362;
	crop_info.height = 3390;   
	...
	for (int index = 1; index <=4; index++)
	{
		...
		{
			(void) sprintf(crop_image->filename, "poster_2x2_A4_crop_%d.bmp", index);
			//(void) sprintf(image_info->filename, "poster_2x2_A4_crop_%d.rgb", index);
        ...
        /*if (crop_image != (Image*) NULL)
        {		
            resize_image = fun_ResizeImage(crop_image, 6780, 9685, LanczosFilter, 1.0, exception);
            if (resize_image != (Image*) NULL)
            {
                (void) sprintf(resize_image->filename, "poster_2x2_A4_%d.bmp", index);
