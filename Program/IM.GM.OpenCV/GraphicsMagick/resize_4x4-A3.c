...
int main(int argc, char **argv)
{
	...
	*/
	...
	image_info->depth = 8;
	CloneString(&(image_info->size), "6780x9685");
    ...
    */
	...
	crop_info.width = 1695;
	crop_info.height = 2421;   
	...
	for (int index = 1; index <=16; index++)
	{
        ...
        */       
        ...
        if (crop_image != (Image*) NULL)
        {		
            resize_image = ResizeImage(crop_image, 6780, 9685, LanczosFilter/*TriangleFilter*/, 1.0, &exception);
            if (resize_image != (Image*) NULL)
            {
                (void) sprintf(resize_image->filename, "poster_4x4_A3_%d.bmp", index);
